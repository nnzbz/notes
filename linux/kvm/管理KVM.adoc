...
network:
  ...
  ethernets:
    enp3s0:     # 替换为你的网卡名
      dhcp4: false
      dhcp6: false
      # 开机时如果网线没插、链路 down，不会阻塞系统启动，不会卡在网络等待超时。做 KVM 宿主机很适合加上
      optional: true
  bridges:
    br0:
      interfaces: [enp3s0]  # 替换为上面的网卡名
      dhcp4: false
      dhcp6: true           # 中国移动可以直接分配 IPv6 地址
      addresses:
      - "xxx.xxx.xxx.xxx/24"  # 替换为你要设置的静态IP
      # 一般会使用供应商提供的 DNS 服务器，不用自己设置
      #nameservers:
      #  addresses:
      #    - 114.114.114.114
      #    - 8.8.8.8
...
[source,shell]
----
# 先运行这个命令，如果没有任何输出，说明 YAML 格式和语法是正确的
sudo netplan generate

# 先测试配置是否正确（有 120 秒回滚保护，防止断网）
sudo netplan try
...
----
# 创建目录
mkdir -p ~/opt/kvm/{data,iso,sh}
# 必须让 libvirt 用户能“穿过”你的个人目录
chmod +x ~
chmod +x ~/opt
chmod +x ~/opt/kvm
chmod +x ~/opt/kvm/data
----
