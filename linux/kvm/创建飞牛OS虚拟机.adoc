...
virt-install \
    ...
    --cdrom /data/kvm/iso/fnos-x.x.x-xxx.iso \
    --disk path=~/opt/kvm/data/fnos-system.raw,format=raw,size=20 \
    --disk path=/data/kvm/raw/fnos-software.raw,format=raw,size=50 \
    --os-variant debian11 \
    --network bridge=br0 \
