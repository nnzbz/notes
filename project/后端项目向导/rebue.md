...
## 再在eclipse里面创建一个本地maven项目，名字于远程仓库一致
...
## 看git笔记，先cd切换到要上传项目的根路径再进行git命令的操作。(ubuntu=>进去项目根目录，显示隐藏目录，复制其他项目根目录的.gitignore文件，
## 这个是git用来忽略不需要提交的文件的，否在将会出现biuld一次就要提交jar和日志文件)
...
### 3：(切记)将参考项目的OldApplication.java改成NewApplication.java，应该还不需要远程调用，将里面所有将有关feigin的删掉。
